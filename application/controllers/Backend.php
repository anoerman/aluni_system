...
class Backend extends CI_Controller {
	...
	*/
	public function signin()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			// Send back data
			$this->session->set_flashdata('signin_info', 'Please check these error!');
			$this->load->view('signin_form');
			$this->load->view('init/init_showpassword');
		}
		else {
			...
			if (count($signin_res->result_array())>0) {
				...
				$this->session->set_flashdata('signin_info', "Wrong identity. Please check your username and your password.");
				$this->load->view('signin_form');
				$this->load->view('init/init_showpassword');
			}
		}
	...
	*/
	public function dashboard()
	{
		...
		if ($status==0) {
			...
			$this->load->view('admin_header', $content);
	...
	*/
	public function data_add_process()
	{
		// Set form validation
			...
			$this->form_validation->set_rules('nickname', 'Nickname', 'trim|required|min_length[3]|callback__username_check');
			$this->form_validation->set_rules('gender', 'Gender', 'trim|required');
			$this->form_validation->set_rules('place_of_birth', 'Place of birth', 'trim|required');
			$this->form_validation->set_rules('date_of_birth', 'Date of birth', 'trim|required');
			$this->form_validation->set_rules('province_city', 'City', 'required');
			$this->form_validation->set_rules('address', 'Address', 'trim');
			// Family
			$this->form_validation->set_rules('partner_name', 'Husband/Wife', 'trim');
			$this->form_validation->set_rules('childrens[]', 'Children', 'trim');
			...
			$this->form_validation->set_rules('parent_address', 'Parent address', 'trim');
			// Contact
			$this->form_validation->set_rules('ext_home_number', 'Home number Regional Number', 'numeric|trim');
			$this->form_validation->set_rules('home_number', 'Home number', 'numeric|trim');
			$this->form_validation->set_rules('phone_number', 'Phone number', 'numeric|trim|required');
			$this->form_validation->set_rules('phone_number_2', 'Phone number 2', 'numeric|trim');
			...
			$this->form_validation->set_rules('website_address', 'Website address', 'valid_url|trim');
			...
			// Academic info
			$this->form_validation->set_rules('generation', 'Generation', 'trim');
			$this->form_validation->set_rules('year_entry', 'Year entry', 'numeric|max_length[4]|required|trim');
			$this->form_validation->set_rules('year_out', 'Year out', 'numeric|max_length[4]|required|trim');
			$this->form_validation->set_rules('last_class', 'Last Class', 'trim');
			$this->form_validation->set_rules('academic_notes', 'Academic Notes', 'trim');
			// Account Info
			$this->form_validation->set_rules('username', 'Username', 'required|trim');
			$this->form_validation->set_rules('password', 'Password', 'required');
		
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('add_data_info', 'Error saving data. Please check these error!');
			// Show add data form
			$content                          = $this->default_system_settings();
			...
			if ($member_id!="") {
				...
				if (!empty($_FILES['photo']['name'])) {
			        ...
			        if ( ! $this->upload->do_upload('photo')) {
						...
						if ($upload_status=="ok") {
							$upload_info = "<br>Photo uploaded!";
						}
						else {
							$upload_info = "<br>Photo upload error!";
						}
			        }
				}
				// Set flashdata info success!
				$this->session->set_flashdata('add_data_info', 'Data successfuly saved!'.$upload_info);
				redirect('backend/data_add', 'refresh');
			}
			// Failed saving
			else {
				$this->session->set_flashdata('add_data_info', 'Error saving data. Please check your input!');
				redirect('backend/data_add', 'refresh');
			}
		}
	}
	...
	*/
	public function _username_check($username)
	{
		...
		if ($check_username!=true) {
			$this->form_validation->set_message('_username_check', 'The {field} is already exist! Please create new username.');
		}
	...
	public function data_edit_process()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			if ($member_id!="") {
				...
				// Set flashdata info success!
				$this->session->set_flashdata('add_data_info', 'Data successfuly saved!'.$upload_info);
				redirect('backend/data_detail/'.$member_id, 'refresh');
			}
			// Failed saving
			else {
				$this->session->set_flashdata('add_data_info', 'Error saving data. Please check your input!');
				redirect('backend/data_edit/$member_id', 'refresh');
			}
		}
	}
	...
	public function user_add_process()
	{
		...
		if ($status==0) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($process) {
					$this->session->set_flashdata("user_info", "Data successfully saved!");
	...
	*/
	public function _user_password_check($password, $username)
	{
		...
		if ($check_password!=true) {
			$this->form_validation->set_message('_user_password_check', 'The {field} you provide is incorrect!');
		}
	...
	*/
	public function user_setting_process()
	{
		...
		if ($status==0) {
			...
			$this->form_validation->set_rules('confirm_new_password', 'Confirm Password', 'matches[new_password]');

			if ($this->form_validation->run() == FALSE) {
				// Set notification
				$this->session->set_flashdata('user_setting_info', 'Error saving data. Please check these error!');
				// Show user setting form
				$content                = $this->default_system_settings();
				$content["user_detail"] = $this->backend_model->user_detail($this->session->userdata("username"));
				$content["module_list"] = $this->backend_model->module_list();
				...
				if ($result) {
					$this->session->set_flashdata('user_setting_info', 'Data successfuly saved!');
					redirect("backend/user_setting", "refresh");
				}
				else {
					$this->session->set_flashdata('user_setting_info', 'Error saving data. Please check these error!');
					redirect("backend/user_setting", "refresh");
	...
	*/
	public function setting_process()
	{
		...
		$this->form_validation->set_rules('system_location', 'System Location', 'trim');

		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata('setting_info', 'Error saving data. Please check these error!');
			// Show system setting form
			$content                 = $this->default_system_settings();
			$content["setting_data"] = $this->home_model->setting_data();
			$this->load->view('admin_header', $content);
			...
			// Redirect
			$this->session->set_flashdata('setting_info', 'Data successfuly saved!');
	...
	*/
	public function report_per_name()
	{
		...
		if ($status==0) {
			...
			$this->load->view('admin_header', $content);
			$this->load->view('report_per_name');
			$this->load->view('admin_footer');
		}
	}
	...
	*/
	public function report_per_regional()
	{
		...
		if ($status==0) {
			...
			$this->load->view('admin_header', $content);
			$this->load->view('report_per_regional');
			$this->load->view('admin_footer');
		}
	}
